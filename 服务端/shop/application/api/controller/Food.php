...
class Food extends Api
{
    protected function initialize()
    {
        ...
        if (!in_array($action, ['index', 'list', 'list2'])) {
            parent::initialize();
        }
    }
    ...
    public function orderlist()
    {
        ...
        foreach ($list as $k => $v) {
            ...
            $result[$k] = [
                ...
                'is_taken' => $v['is_taken'],
                'comment' => $v['comment'],
                'delivery_type' => isset($v['delivery_type']) ? $v['delivery_type'] : 'dine',
                'create_time' => $v['create_time'],
                'pay_time' => $v['pay_time'],
                'first_food_name' => FoodModel::where('id', $food_id)->value('name')
            ];
        }
        return json(['list' => $result, 'last_id' => $last_id]);
    }
    ...
    protected function createOrder()
    {
        ...
        $comment = $this->request->post('comment/s', '', 'trim');
        $deliveryType = $this->request->post('deliveryType/s', 'dine');
        $food_ids = [];
        foreach ($order as $v) {
